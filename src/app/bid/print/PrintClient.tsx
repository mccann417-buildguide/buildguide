...
function BidPrintInner() {
  ...
  React.useEffect(() => {
    ...
    const b = safeParse<BidAnalysisResult>(
      localStorage.getItem(baseKey(rid))
    );
    const d = safeParse<BidDetailAI>(
      localStorage.getItem(detailKey(rid))
    );
      ...
      {!base ? (
        <div className="rounded border p-4 text-sm">
          ...
          <div className="mt-2 text-neutral-600">
            (MVP note: this print page reads localStorage. If you need
            cross-device printing, we’ll store reports on the server.)
          </div>
        </div>
      ) : (
        <>
          ...
          {detail ? (
              ...
              {detail.marketComparison && (
                <section className="space-y-2">
                  ...
                  <div className="text-sm">
                    <div>
                      <b>Area:</b>{" "}
                      {detail.marketComparison.area}
                    </div>
                    <div>
                      <b>Expected range:</b>{" "}
                      {detail.marketComparison.expectedRange.low} ·{" "}
                      {detail.marketComparison.expectedRange.mid} ·{" "}
                      {detail.marketComparison.expectedRange.high}
                    </div>
                    <div>
                      <b>Verdict:</b>{" "}
                      {detail.marketComparison.verdict}
                    </div>
            ...
            <div className="rounded border p-4 text-sm">
              ...
              <div className="mt-2 text-neutral-600">
                Generate the Full Report once, then this page will
                include it.
              </div>
            </div>
          )}
